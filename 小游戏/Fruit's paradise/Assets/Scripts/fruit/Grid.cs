...
public class Grid : MonoBehaviour {
    public int xDim;
    public int yDim;
    public float fillTime;//下落时间
    public Level level;
    public PiecePositon[] initialPieces;
	...
	void Awake () {
        piecePrefabDict = new Dictionary<PieceType, GameObject>();//生成网格
        ...
        pieces = new GamePiece[xDim, yDim];//生成游戏对象
                    ...
                    else
                    {
                        for(int diag=-1;diag<=1;diag++)
                        {
                            if(diag!=0)
                            {
                                ...
                                {
                                    GamePiece diagonaPiece = pieces[diagX, y + 1];
                                    if(diagonaPiece.Type==PieceType.EMPTY)
                                    {
                                        ...
                                        if(!hasPieceAbove)
                                        {
                                            Destroy(diagonaPiece.gameObject);
    ...
    public bool IsAdjacent(GamePiece piece1,GamePiece piece2)//判断两个元素是否相邻并且是同一行或者同一列
    {
    ...
    public void PressPiece(GamePiece piece)//按下鼠标
    {
        pressedPiece = piece;
    }
    public void EnterPiece(GamePiece piece)//进入鼠标悬停
    {
        enteredPiece = piece;
    }
    public void ReleasePiece()//松开鼠标交换两个位置
    {
        if(IsAdjacent(pressedPiece,enteredPiece))
        {
            SwapPieces(pressedPiece, enteredPiece);
            AudioManager.instance.playExchange();
        }
    }
    public List<GamePiece> GetMatch(GamePiece piece,int newX,int newY)
    {
        if(piece.IsColored())
        {
            ...
            List<GamePiece> horizontalPieces = new List<GamePiece>();//水平检查
            List<GamePiece> verticalPieces = new List<GamePiece>();//垂直检查
            ...
            if(horizontalPieces.Count>=3)
            {
                ...
                {
                    matchPieces.Add(horizontalPieces[i]);//matchingPieces
                }
            }//查找水平的
            if(horizontalPieces.Count>=3)
            {
    ...
    public bool ClearPiece(int x,int y)
    {
        if (pieces[x, y].IsClearable() && !pieces[x, y].ClearableComponent.IsBeingCleared)
        {
            ...
            SpawnNewPiece(x, y, PieceType.EMPTY);
            ClearObstacles(x, y);
            //AudioManager.instance.playGreat();
            return true;
        }
        return false;
    }
